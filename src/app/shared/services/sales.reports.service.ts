...
export class SalesReportsService {
    ...
    getSalesOrdersReport(page = 1, perPage = 10, name: string, seller: string, dateIni?: Date | null, dateEnd?: Date | null){
        return this.http.get(`${environment.apiBaseURL}/sales/reports/sales-orders?pageNumber=${page}&perPage=${perPage}&name=${name}&seller=${seller}&dateIni=${dateIni?.toISOString()}&dateEnd=${dateEnd?.toISOString()}`)
    }
